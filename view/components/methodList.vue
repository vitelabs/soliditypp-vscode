<template>
    <div>
        ...
        <div
            v-for="(functionAbi, functionAbiIndex) in functionAbiList"
        ...
        >
            <h5 class="title">
                <i
                    ...
                    @click="hideFunctionAbi(functionAbi)"
                    v-if="functionAbi.isShow"
                ></i>
                <i class="el-icon-caret-bottom" @click="showFunctionAbi(functionAbi)" v-else></i>
                <template v-if="functionAbi.abi.type === 'offchain'">offchain {{functionAbi.abi.name}}</template>
                <template v-else>{{functionAbi.abi.name}}</template>
            </h5>
            <template v-if="functionAbi.isShow">
                ...
                <el-row
                    ...
                    align="middle"
                    :key="input.name"
                    v-for="(input, index) in functionAbi.abi.inputs"
                >
                ...
                <template v-if="functionAbi.result">
                    <el-row
                        ...
                        align="middle"
                        :key="item.name"
                        v-for="item in functionAbi.result"
                    >
    ...
    methods: {
        ...
        async callContract(functionAbi) {
            try {
                ...
                let contractBlock = await client.request(
                ...
                this.$message({
                    ...
                    }" failed, error is ${JSON.stringify(err)}`,
                    type: 'error'
                });
            }
        ...
        async callOffchain(functionAbi) {
            try {
                ...
                this.$message({
                    message: `Call offchain "${
                        functionAbi.abi.name
                    }" success!"${offchainResult}"`,
                    type: 'success'
                });
