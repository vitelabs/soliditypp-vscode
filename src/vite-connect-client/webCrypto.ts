...
import {
  ...
  export async function exportKey (cryptoKey: CryptoKey): Promise<ArrayBuffer> {
    const buffer: ArrayBuffer = await webcrypto.subtle.exportKey(
      'raw',
      cryptoKey
    )
    return buffer
  ...
  export async function aesCbcEncrypt (
    ...
    const result: ArrayBuffer = await webcrypto.subtle.encrypt(
      {
        iv,
        name: AES_ALGORITHM
      },
  ...
  export async function aesCbcDecrypt (
    ...
    const cryptoKey: CryptoKey = await importKey(key, AES_ALGORITHM)
    const result: ArrayBuffer = await webcrypto.subtle.decrypt(
      {
        iv,
        name: AES_ALGORITHM
      },
